...
function FinanceiroPage() {
  ...
  async function load() {
    const [{ data: cp }, { data: cr }, { data: f }, { data: c }] = await Promise.all([
      supabase
        .from("contas_pagar")
        .select("id, descricao, fornecedor_id, valor, data_vencimento, data_pagamento, status")
        .order("data_vencimento"),
      supabase
        .from("contas_receber")
        .select("id, descricao, cliente_id, valor, data_vencimento, data_recebimento, status")
        .order("data_vencimento"),
  ...
  const kpis = useMemo(() => {
    const abertoPagar = pagar
      .filter((p) => p.status === "aberto" || p.status === "parcial" || p.status === "vencido")
      .reduce((s, p) => s + Number(p.valor), 0);
    const abertoReceber = receber
      .filter((r) => r.status === "aberto" || r.status === "parcial" || r.status === "vencido")
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs text-muted-foreground">A receber</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold text-success flex items-center gap-2">
              <TrendingUp className="h-5 w-5" />
              {brl(kpis.abertoReceber)}
            </p>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <CardContent>
            <p
              className={`text-2xl font-bold ${kpis.saldo >= 0 ? "text-success" : "text-destructive"}`}
            >
              {brl(kpis.saldo)}
            </p>
          </CardContent>
        ...
        <TabsContent value="receber">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              ...
              {pode && (
                <Dialog open={openCr} onOpenChange={setOpenCr}>
                ...
                <TableBody>
                  {receber.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className={TITULO_STATUS_TONE[r.status]}>
                          {TITULO_STATUS_LABEL[r.status]}
                        </Badge>
                      </TableCell>
                    </TableRow>
                  ))}
        ...
        <TabsContent value="pagar">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              ...
              {pode && (
                <Dialog open={openCp} onOpenChange={setOpenCp}>
                  <DialogTrigger asChild>
                    <Button size="sm">
                      <Plus className="mr-2 h-4 w-4" />
                      Novo
                ...
                <TableBody>
                  {pagar.map((p) => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className={TITULO_STATUS_TONE[p.status]}>
                          {TITULO_STATUS_LABEL[p.status]}
                        </Badge>
                      </TableCell>
                    </TableRow>
                  ))}
...
function TituloForm({
  ...
  return (
    <DialogContent>
      <DialogHeader>
        <DialogTitle>Novo título — {tipo === "pagar" ? "Pagar" : "Receber"}</DialogTitle>
      </DialogHeader>
      <form onSubmit={submit} className="space-y-3">
        ...
        <div>
          ...
          <Select value={f.status} onValueChange={(v) => setF({ ...f, status: v as TituloStatus })}>
            ...
            <SelectContent>
              {(Object.keys(TITULO_STATUS_LABEL) as TituloStatus[]).map((s) => (
                <SelectItem key={s} value={s}>
                  {TITULO_STATUS_LABEL[s]}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
