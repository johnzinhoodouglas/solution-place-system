...
function SegurancaPage() {
  ...
  async function loadAll() {
    const [e, en, i, d] = await Promise.all([
      ...
      supabase
        .from("entregas_epi")
        .select("id, epi_id, colaborador_nome, quantidade, data_entrega, assinado, epi:epis(nome)")
        .order("data_entrega", { ascending: false })
        .limit(100),
      supabase
        .from("incidentes_seguranca")
        .select("id, data_ocorrencia, tipo, gravidade, local, descricao, dias_afastamento")
        .order("data_ocorrencia", { ascending: false }),
      supabase
        .from("dds")
        .select("id, data, tema, responsavel_nome, qtd_participantes")
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              EPIs, incidentes e DDS — conformidade com NRs.
            </p>
          </div>
        </div>
      </div>
            ...
            <div key={i.id} className="rounded-lg border border-border p-3 text-sm">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className={INC_GRAV_TONE[i.gravidade]}>
                    {INC_GRAV_LABEL[i.gravidade]}
                  </Badge>
                  <span className="font-medium">{INC_TIPO_LABEL[i.tipo]}</span>
                  {i.dias_afastamento > 0 && (
                    <Badge variant="outline">Afastamento: {i.dias_afastamento}d</Badge>
                  )}
                </div>
...
function IncidenteForm({ userId, onSaved }: { userId: string | null; onSaved: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("incidentes_seguranca").insert({
      ...
      descricao,
      local: local || null,
      dias_afastamento: Number(dias) || 0,
      registrado_por: userId,
    });
    setSaving(false);
    if (error) return toast.error(error.message);
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
        ...
        {pode && (
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button size="sm">
                <Plus className="mr-2 h-4 w-4" /> Novo DDS
              </Button>
            </DialogTrigger>
            <DdsForm
              onSaved={() => {
...
function DdsForm({ onSaved }: { onSaved: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("dds").insert({
      ...
      responsavel_nome: resp || null,
      qtd_participantes: Number(qtd) || 0,
    });
    setSaving(false);
    if (error) return toast.error(error.message);
