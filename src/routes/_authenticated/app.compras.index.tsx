  ...
  async function load() {
    ...
    const [{ data: f }, { data: p }] = await Promise.all([
      ...
      supabase
        .from("pedidos_compra")
        .select(
          "id, numero, fornecedor_id, valor_total, status, data_pedido, data_prev_entrega, observacoes",
        )
                  ...
                  <TableBody>
                    {peds.map((p) => (
                      <TableRow key={p.id}>
                        ...
                        <TableCell>
                          <Badge variant="outline">{PC_STATUS_LABEL[p.status]}</Badge>
                        </TableCell>
                        ...
                        <TableCell className="text-xs">
                          {p.data_prev_entrega
                            ? new Date(p.data_prev_entrega).toLocaleDateString("pt-BR")
                            : "—"}
                        </TableCell>
                      </TableRow>
                    ))}
...
function PedidoForm({
  ...
  const [f, setF] = useState({
    ...
    data_prev_entrega: "",
    observacoes: "",
    status: "rascunho" as PedidoCompraStatus,
  });
  const [saving, setSaving] = useState(false);
  async function submit(e: React.FormEvent) {
    e.preventDefault();
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input
              type="date"
              value={f.data_prev_entrega}
              onChange={(e) => setF({ ...f, data_prev_entrega: e.target.value })}
            />
          </div>
        </div>
        <div>
          ...
          <Select
            value={f.status}
            onValueChange={(v) => setF({ ...f, status: v as PedidoCompraStatus })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {(Object.keys(PC_STATUS_LABEL) as PedidoCompraStatus[]).map((s) => (
