...
import { brl, ORC_STATUS_LABEL, ORC_STATUS_TONE, CT_STATUS_LABEL, podeGerirVendas, type OrcamentoStatus, type ContratoStatus } from "@/lib/comercial";

export const Route = createFileRoute("/_authenticated/app/vendas/")({ component: VendasPage });

type Cliente = { id: string; nome: string; documento: string | null; email: string | null; telefone: string | null; cidade: string | null; uf: string | null; ativo: boolean };
type Orc = { id: string; numero: string; cliente_id: string | null; descricao: string | null; valor_total: number; status: OrcamentoStatus; validade: string | null; created_at: string };
type Contrato = { id: string; numero: string; cliente_id: string | null; objeto: string; valor: number; status: ContratoStatus; data_inicio: string | null };
...
function VendasPage() {
  ...
  async function load() {
    ...
    const [{ data: c }, { data: o }, { data: t }] = await Promise.all([
      ...
      supabase.from("orcamentos").select("id, numero, cliente_id, descricao, valor_total, status, validade, created_at").order("created_at", { ascending: false }),
      supabase.from("contratos").select("id, numero, cliente_id, objeto, valor, status, data_inicio").order("created_at", { ascending: false }),
    ]);
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Clientes, orçamentos e contratos comerciais.</p>
        </div>
      </div>
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              ...
              {pode && (
                <Dialog open={openOrc} onOpenChange={setOpenOrc}>
                  ...
                  <OrcForm clientes={clientes} userId={user?.id ?? null} onSaved={() => { setOpenOrc(false); load(); }} />
                </Dialog>
              )}
            </CardHeader>
            <CardContent>
              {busy ? <p className="text-sm text-muted-foreground">Carregando…</p> : (
                <Table>
                  <TableHeader><TableRow>
                    <TableHead>Nº</TableHead><TableHead>Cliente</TableHead><TableHead>Descrição</TableHead>
                    <TableHead>Valor</TableHead><TableHead>Status</TableHead><TableHead>Validade</TableHead>
                  ...
                  <TableBody>
                    {orcs.map((o) => (
                      <TableRow key={o.id}>
                        ...
                        <TableCell><Badge variant="outline" className={ORC_STATUS_TONE[o.status]}>{ORC_STATUS_LABEL[o.status]}</Badge></TableCell>
                        <TableCell className="text-xs">{o.validade ? new Date(o.validade).toLocaleDateString("pt-BR") : "—"}</TableCell>
                      </TableRow>
                    ))}
                    {orcs.length === 0 && <TableRow><TableCell colSpan={6} className="text-center text-sm text-muted-foreground">Nenhum orçamento.</TableCell></TableRow>}
                  </TableBody>
                </Table>
              )}
            </CardContent>
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              ...
              {pode && (
                <Dialog open={openCt} onOpenChange={setOpenCt}>
                  <DialogTrigger asChild><Button size="sm"><Plus className="mr-2 h-4 w-4" />Novo</Button></DialogTrigger>
                  <ContratoForm clientes={clientes} userId={user?.id ?? null} onSaved={() => { setOpenCt(false); load(); }} />
                </Dialog>
              )}
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader><TableRow>
                  <TableHead>Nº</TableHead><TableHead>Cliente</TableHead><TableHead>Objeto</TableHead>
                  <TableHead>Valor</TableHead><TableHead>Status</TableHead><TableHead>Início</TableHead>
                </TableRow></TableHeader>
                <TableBody>
                  {contratos.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell><Badge variant="outline">{CT_STATUS_LABEL[c.status]}</Badge></TableCell>
                      <TableCell className="text-xs">{c.data_inicio ? new Date(c.data_inicio).toLocaleDateString("pt-BR") : "—"}</TableCell>
                    </TableRow>
                  ))}
                  {contratos.length === 0 && <TableRow><TableCell colSpan={6} className="text-center text-sm text-muted-foreground">Nenhum contrato.</TableCell></TableRow>}
                </TableBody>
              </Table>
            </CardContent>
          </Card>
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              ...
              {pode && (
                <Dialog open={openCli} onOpenChange={setOpenCli}>
                  <DialogTrigger asChild><Button size="sm"><Plus className="mr-2 h-4 w-4" />Novo</Button></DialogTrigger>
                  <ClienteForm userId={user?.id ?? null} onSaved={() => { setOpenCli(false); load(); }} />
                </Dialog>
              )}
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader><TableRow>
                  <TableHead>Nome</TableHead><TableHead>Documento</TableHead><TableHead>Contato</TableHead><TableHead>Cidade</TableHead>
                </TableRow></TableHeader>
                <TableBody>
                  {clientes.map((c) => (
                    <TableRow key={c.id}>
                  ...
                  {clientes.length === 0 && <TableRow><TableCell colSpan={4} className="text-center text-sm text-muted-foreground">Nenhum cliente.</TableCell></TableRow>}
                </TableBody>
              </Table>
            </CardContent>
          </Card>
...
function ClienteForm({ onSaved, userId }: { onSaved: () => void; userId: string | null }) {
  const [f, setF] = useState({ nome: "", documento: "", tipo_pessoa: "PJ", email: "", telefone: "", cidade: "", uf: "" });
...
function OrcForm({ clientes, onSaved, userId }: { clientes: Cliente[]; onSaved: () => void; userId: string | null }) {
  const [f, setF] = useState({ cliente_id: "", descricao: "", valor_total: "", validade: "", status: "rascunho" as OrcamentoStatus });
  const [saving, setSaving] = useState(false);
  async function submit(e: React.FormEvent) {
    e.preventDefault(); setSaving(true);
    const { error } = await supabase.from("orcamentos").insert({
      numero: "", cliente_id: f.cliente_id || null, descricao: f.descricao || null,
      valor_total: Number(f.valor_total) || 0, validade: f.validade || null,
      status: f.status, created_by: userId,
    });
    setSaving(false);
    if (error) return toast.error(error.message);
    toast.success("Orçamento criado"); onSaved();
  }
  return (
    <DialogContent>
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div><Label>Status</Label>
          <Select value={f.status} onValueChange={(v)=>setF({...f, status:v as OrcamentoStatus})}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>{(Object.keys(ORC_STATUS_LABEL) as OrcamentoStatus[]).map(s=><SelectItem key={s} value={s}>{ORC_STATUS_LABEL[s]}</SelectItem>)}</SelectContent>
          </Select>
        </div>
...
function ContratoForm({ clientes, onSaved, userId }: { clientes: Cliente[]; onSaved: () => void; userId: string | null }) {
  const [f, setF] = useState({ cliente_id: "", objeto: "", valor: "", data_inicio: "", data_fim: "" });
  const [saving, setSaving] = useState(false);
  async function submit(e: React.FormEvent) {
    e.preventDefault(); setSaving(true);
    const { error } = await supabase.from("contratos").insert({
      numero: "", cliente_id: f.cliente_id || null, objeto: f.objeto,
      valor: Number(f.valor) || 0, data_inicio: f.data_inicio || null, data_fim: f.data_fim || null,
      created_by: userId,
    });
    setSaving(false);
    if (error) return toast.error(error.message);
  ...
  return (
    <DialogContent>
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div><Label>Objeto *</Label><Textarea required rows={2} value={f.objeto} onChange={(e)=>setF({...f, objeto:e.target.value})} /></div>
